...
class Article_model extends MY_Model
{
    ...
    { 
		...
		$column_order = array(
            $table_article_i18 . '.article_site_id',
			$table_user . '.user_fname',
            $table_article_i18 . '.article_title',
            $table_article_i18 . '.article_content_score',
            $table_article_i18 . '.article_backlinks_count',
			$table_article . '.article_type',
			...
			foreach ($post_array['columns'] as $key=>$column_array)
			{
				if($column_array['searchable'] && $column_array['search']['value']){
                    ...
                    if($column_array['data'] =='article_status' && $column_array['search']['value']==null ){
                        $this->db->where($table_article_i18.'.article_status !=', 'deleted');
