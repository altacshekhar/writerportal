...
foreach($links_data as $links)
{
    ...
    <div class="alert alert-secondary alert-default d-flex mt-1 m-0">
        <div>
        <p class="text-justify"><?php echo $a_desc; ?></p>
        <span style="display: inherit; margin-top: 8px; opacity: 0.6;">
            <i class="fas fa-link"></i>
            <?php 
            $result = parse_url($links['a_href']);
            $href_url = $result['scheme']."://".$result['host'];
            ?>
            <span style="vertical-align: middle;">External link to <a href="<?php echo $links['a_href']?>" target="_blank"><u class="ng-binding"><?php echo $href_url ?></u></a></span>
        </span>
        </div>
