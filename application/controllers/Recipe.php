...
class Recipe extends Frontend_Controller
{
    ...
    public function index($id = null, $lang = 'en')
    {
      ...
      //pre_exit($social_channels);
        ...
        if (count($paragraphs) <1 ) {
            ...
            $paragraphs[0]['ingredients'][0] = $this->ingredients_i18_model->get_new();
            $paragraphs[0]['steps'][0] = $this->steps_i18_model->get_new();
        }
       ...
       /* $this->data['twitters'] = $twitters;
        ...
        $this->data['category'] = $this->category_model->get_category_by_array('recipe');
           ...
           if (!empty($paragraphs)) {
                foreach ($paragraphs as $paragraph) {
					...
					if($paragraph['section_title'] || $paragraph['section_image_base64'] || trim($paragraph['section_text'])){
						...
						unset($data_paragraph_i18['ingredients']);
						unset($data_paragraph_i18['steps']);
			            ...
			            $ingredients = array();
						if(array_key_exists('ingredients', $paragraph)){
							$ingredients = $paragraph['ingredients'];
                        }
                        //pre($ingredients);
						$this->ingredients_i18_model->save_article_i18_ingredients($ingredients, $section_id, $lang);
						$steps = array();
						if(array_key_exists('steps', $paragraph)){
							$steps = $paragraph['steps'];
                        }
                        //pre($steps);
						$this->steps_i18_model->save_article_i18_steps($steps, $section_id, $lang);
					}
                }
           }
            /*** Paragraphs End ****/
				...
				$msg_info = $this->promotion_model->get_promo_message_id($article_last_insert_id, $lang, $channel_name);
                                ...
                                $social_data_save['msg_url_link'] = 'https://' . $article['article_site_id'] . '/recipes/' . slugify($this->input->post("article[$lang][article_title]")) . '.html';
        ...
        $this->data['subview'] = 'recipe';
		$this->data['nestedview'] = 'recipe_i18';
		$this->data['meta_title'] = 'Recipe';
