...
class Campaigns extends Admin_Controller
{
    ...
    public function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() === true) {
			...
			if (!$id) {
				...
				//$data['campaign_status'] = 'Waiting for outreach coordinator to assign publishers';
			}
            ...
            if(!$id){
				...
				if($user_id){
					...
					$htmlContent .= '<p>A campaign has been created by ' . $this->session->userdata('email')  . '.</p>';
					...
					$emailer_data['message_subject'] = 'Campaign Created For {portal_name} Writer Portal';
		...
		foreach ($backlinks as $key => $backlink) {
			...
			foreach ($article_wp_url as $wp_url_key => $wp_url) {
				...
				//pre($backlink['article_anchortext'][$wp_url_key],"article_anchortext");
	...
	public function ajax_list()
    {
        ...
        foreach ($campaign_rows as $campaign_row) {
			...
			$row[] 	= $publisher_cnt.' / '.$total_publisher;
	...
	public function articles_build_backlinks_list()
	{
		...
		if(count($websites)>1){
			...
			$this->db->where('article_site_id', $websites[0]);
		}
		...
		foreach ($result_array  as $result) {
			$article_type = $this->get_article_type($result['article_id']);
			switch ($article_type) {
				case "pillar":
				...
				case "supporting":
				  ...
				  break;
				case "recipe":
					$article_url =  'https://' . $article['article_site_id'] . '/recipes/' . slugify($result['article_title']) . '.html';
					$keywords ='';
				  break;
				case "news":
	...
	public function delete_backlink(){
		...
		$backlink_ar = explode(',',$backlink_ids);
		foreach($backlink_ar as $backlink_id)
		{
			if($backlink_id > 0){
	...
	public function publishers_list(){
		...
		if(count($niches)>1){
			$this->db->group_start();
			foreach ($niches  as $niche) {
				$this->db->or_like('link_publishers.publisher_niche', $niche);
			}
			$this->db->group_end();
		}else{
			$this->db->like('link_publishers.publisher_niche', $niches[0]);
		}
		if(count($types)>1){
			$this->db->group_start();
			...
			$this->db->like('link_publishers.publisher_type', $types[0]);
		}
		if($added_publishers)
	...
	public function get_all_backlink()
	{
		...
		pre($data['campaign_briefs'],'campaign_briefs');
		//echo "backlink_list";
		//pre($data['backlink_list'],"backlink_list");
		//echo "backlink_url_list";
		//pre($data['backlink_url_list'],"backlink_url_list");
		//pre($data['campaign_quantity'],"campaign_quantity");
		//pre($template);
        if($template =='col'){			
			$this->load->view('secure/campaign/backlink_template_column', $data);
		}else{
